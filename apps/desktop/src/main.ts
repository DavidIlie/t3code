...
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "T3 Gurt (Dev)" : "T3 Gurt";
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `T3 Gurt ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/T3 Gurt (Alpha)` on Linux). This is
