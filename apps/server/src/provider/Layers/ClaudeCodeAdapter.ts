 ...
 *
 * - Strips `CLAUDECODE` / `CLAUDE_CODE_ENTRYPOINT` so the SDK does not
 *   refuse to launch inside an existing Claude Code session.
...
/**
 * Resolve the `executable` option for the SDK's `ProcessTransport`.
 *
 * Inside an Electron packaged app `node` may not be on PATH. Use the
 * Electron binary itself (with `ELECTRON_RUN_AS_NODE=1` in env) as a
 * fully-functional Node.js runtime.
 */
function resolveClaudeExecutable(): "node" | "bun" {
  // Bun runtime — use bun
 ...
 *
 * Priority:
 * 1. System-installed `claude` binary (avoids asar/node issues entirely)
 * 2. SDK's bundled `cli.js` with asar-unpacked rewrite for Electron
 * 3. undefined (let the SDK resolve internally — works outside Electron)
 */
function resolveClaudeCliPath(): string | undefined {
  // Strategy 1: Use the system-installed claude binary if available.
  ...
  // child_process.spawn cannot access — rewrite to app.asar.unpacked.
  try {
    ...
    if (sdkUrl) {
      sdkEntry = fileURLToPath(sdkUrl) as string;
    } else {
    ...
    if (sdkEntry) {
      ...
      if (cliPath.includes("app.asar")) {
        return cliPath.replace(/app\.asar(?!\.unpacked)/, "app.asar.unpacked");
      }
      return cliPath;
    }
  } catch {
...
/**
 * A tool that has finished streaming (content_block_stop) but hasn't yet
 * been through the canUseTool callback, which is the only place with the
 * complete tool input.
 */
...
interface ClaudeSessionContext {
  ...
  /** Tools awaiting canUseTool for the full input before emitting item.completed. */
  readonly pendingToolCompletions: Map<string, PendingToolCompletion>;
...
function buildUserMessage(
  ...
  return Effect.gen(function* () {
    ...
    if (input.input && input.input.trim().length > 0) {
      content.push({ type: "text", text: input.input.trim() });
    }
    ...
    for (const attachment of input.attachments ?? []) {
      if (attachment.type === "image") {
        ...
        if (!attachmentPath) {
          content.push({
            type: "text",
            text: `Attached image: ${attachment.name} (${attachment.mimeType}, ${attachment.sizeBytes} bytes) — could not resolve file.`,
          });
          continue;
        }
          ...
          Effect.mapError(
            (cause) =>
              new ProviderAdapterRequestError({
                provider: PROVIDER,
                method: "turn/start",
                detail: `Failed to read attachment file: ${attachmentPath}`,
                cause,
              }),
          ),
        );
  ...
  return Effect.gen(function* () {
    ...
    const handleStreamEvent = (
      ...
      Effect.gen(function* () {
        ...
        if (event.type === "content_block_delta") {
          // Accumulate tool input JSON fragments for display at content_block_stop.
          if (
        ...
        if (event.type === "content_block_stop") {
          ...
          if (tool.inputJsonFragments && tool.inputJsonFragments.length > 0) {
            try {
              ...
              // Keep existing detail if JSON parse fails.
            }
          }

          // Store for canUseTool to emit a corrected item.completed with full input.
          context.pendingToolCompletions.set(tool.itemId, {
            tool,
            providerRefs: {
              ...providerThreadRef(context),
              ...(context.turnState ? { providerTurnId: String(context.turnState.turnId) } : {}),
      ...
      Effect.gen(function* () {
        ...
        const canUseTool: CanUseTool = (toolName, toolInput, callbackOptions) =>
          Effect.runPromise(
            Effect.gen(function* () {
              ...
              // Emit item.updated with the full tool input detail.
              // content_block_stop already emitted item.completed but may
              // have had an empty input ({}).  canUseTool always receives
              // the fully-assembled input.
              const pendingCompletion = context.pendingToolCompletions.get(
                callbackOptions.toolUseID,
              );
              if (pendingCompletion) {
                context.pendingToolCompletions.delete(callbackOptions.toolUseID);
                const fullDetail = summarizeToolRequest(toolName, toolInput);
                if (fullDetail) {
                  ...
                  yield* offerRuntimeEvent({
                    ...
                    ...(context.turnState
                      ...
                      : {}),
                    itemId: asRuntimeItemId(pendingCompletion.tool.itemId),
                    payload: {
                      itemType: pendingCompletion.tool.itemType,
                      status: "completed",
                      title: pendingCompletion.tool.title,
                      detail: fullDetail,
                    },
                    providerRefs: {
                      ...providerThreadRef(context),
                    ...
                    raw: {
                      ...
                      method: "canUseTool/item.updated",
        ...
        // Fetch supported commands (skills) and emit them via session.configured
        {
          ...
          if (commands.length > 0) {
            ...
            yield* offerRuntimeEvent({
              type: "session.configured",
              eventId: commandsStamp.eventId,
              provider: PROVIDER,
              createdAt: commandsStamp.createdAt,
              threadId,
              payload: {
                config: { commands },
              },
    ...
    const sendTurn: ClaudeCodeAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        // The Claude Code SDK natively supports a "plan" permission mode that
        // restricts Claude to thinking/planning without executing tools.
        if (input.interactionMode === "plan") {
          yield* Effect.tryPromise({
            try: () => context.query.setPermissionMode("plan"),
            catch: (cause) => toRequestError(input.threadId, "turn/setPermissionMode", cause),
          });
        } else if (input.interactionMode === "default" || input.interactionMode === undefined) {
          // Restore the session's original permission mode when returning to
          // default interaction mode (in case a previous turn was in plan mode).
          const originalMode: PermissionMode =
            context.session.runtimeMode === "full-access" ? "bypassPermissions" : "default";
          yield* Effect.tryPromise({
            try: () => context.query.setPermissionMode(originalMode),
            catch: (cause) => toRequestError(input.threadId, "turn/setPermissionMode", cause),
          });
        }
